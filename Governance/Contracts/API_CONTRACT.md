...
Define the minimum local programmatic contract required for
`M18 Runtime API`.

This contract documents the public operation vocabulary, input
boundary, output boundary, lifecycle-control boundary, validation
behavior, and error behavior required by the published work item.

---

# Repository Evidence

`Governance/WorkQueue/M18-Runtime-API.md` requires a Runtime API that:
...
- `ADR-0013` limits the API to a local in-process programmatic surface
  over published runtime boundaries.
...
- the logical Runtime API operation vocabulary
- operation input validation behavior
- deterministic operation result behavior
- lifecycle control of a caller-managed active pipeline

This contract does not define networking, server deployment, remote
execution, cross-process session behavior, or authentication.

---

# Operation Boundary

Exact Python signatures are not published by current repository
evidence.

The approved Runtime API operation vocabulary and behavior boundaries
are:

| Operation | Input Boundary | Output Boundary | Required Behavior |
| --- | --- | --- | --- |
| `inspect` | An operation input containing a required `target` of `plugins`, `applications`, `engines`, `stages`, or `runtime` | A deterministic result containing a serialized snapshot for the requested target | Inspection is read-only and does not mutate runtime state |
| `validate` | An operation input targeting a published pipeline definition or caller-managed pipeline instance | A deterministic result containing validation results and the current pipeline snapshot | Validation uses the published pipeline and validation boundaries and does not bypass schema or dependency checks |
| `execute` | An operation input targeting a published pipeline definition or caller-managed pipeline instance | A deterministic result containing ordered stage execution records and the resulting pipeline snapshot | Execution performs validation before stage execution begins |
| `execute-stage` | An operation input containing a required `stage_id` plus a published pipeline definition or caller-managed pipeline instance | A deterministic result containing ordered execution records for the requested stage closure and the resulting pipeline snapshot | Targeted execution does not bypass validation or dependency requirements |
| `pause` | An operation input targeting a caller-managed active pipeline instance | A deterministic result containing the resulting runtime state and pipeline snapshot | `pause` is valid only while the published pipeline runtime is `Running` |
| `resume` | An operation input targeting a caller-managed paused pipeline instance | A deterministic result containing the resulting runtime state and pipeline snapshot | `resume` is valid only while the published pipeline runtime is `Paused` |
| `stop` | An operation input targeting a caller-managed running or paused pipeline instance | A deterministic result containing the resulting runtime state and pipeline snapshot | `stop` is valid only while the published pipeline runtime is `Running` or `Paused` |

---

# Input Boundary

Runtime API operation input shall validate against:

- `AXI-SCH-014 API Operation`

The published operation payload contains:
...
This contract does not publish service bootstrap payloads, cross-
process session identifiers, or transport-specific metadata.

---

# Output Boundary

Successful operation execution shall return a deterministic result
object containing:
...
This contract does not require new API-specific exception classes.

Repository-backed error expectations are:

- unsupported operations shall fail validation before runtime dispatch
- unsupported `inspect` targets shall fail validation or operation
  dispatch before runtime mutation
- missing `stage_id` for `execute-stage` shall fail validation or
  operation dispatch before runtime mutation
...
`M18` requires the API to reuse existing platform exceptions whenever
appropriate.
...
- exact language-binding details
- concrete exception-to-transport mappings
- persisted session behavior
- remote API transport
- richer result envelopes
