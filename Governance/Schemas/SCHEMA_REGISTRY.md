...
`AXI-SCH-022` now includes `Transition Gate` and
`Prompt Operations Manual` as governed publication types so
constitutional phase-change controls and prompt-routing governance
remain first-class repository artifacts rather than narrative-only
directives.
